...
newcol = []
SKIP = 0
# first, connect all hyphenated words (keeping POS of first piece)
for i,l in enumerate(newcolFile):
    ...
    word = l.strip().split()
    if '-' == word[1]:
        newcol[-1][1] = newcol[-1][1] + word[1] + newcolFile[i+1].strip().split()[1]
        SKIP = 1
        continue
...
# now add the depth of the words to the table
wordix = 0
falsebadparses = [1]
badparseix = 0
inBAD = False
...
for tline in tableFile:
    ...
    if inBAD and '<SENT-END>' == stline[3]:
        ...
        # but then the depth would be off anyway
        inBAD = False
    if stline[0][0] == '#':
        #in a comment line
        stline.append('syndepth')
        ...
        stline.append(newcol[wordix-1][0][-1])
        sys.stdout.write('\t'.join(stline)+'\n')
        continue
    ...
    while americanize(badchars.sub('',newcol[wordix][1].lower())) != badchars.sub('',stline[4].lower()):
        ...
        if DEBUG:
            sys.stderr.write(str(wordix)+'/'+str(len(newcol))+': '+str(newcol[wordix])+' ?= ')
            sys.stderr.write(str(stline[4])+'\n')
    stline.append(newcol[wordix][0][-1])
    sys.stdout.write('\t'.join(stline)+'\n')
    wordix += 1
