# addmorphs.py tableFile newcolFile > newtable
# newcolFile has two columns: morphcount word
# TODO: the morph data doesn't need to be tied to the parser;
#       splitting the two would give morphs more data
...
newcol = []
SKIP = 0
# first, connect all hyphenated words (summing morph counts of pieces)
for i,l in enumerate(newcolFile):
    ...
    if '-' == word[1]:
        ...
        newcol[-1][0] = str(float(newcol[-1][0]) + float(word[0])) #sum morphs over all hyphenated components
        SKIP = 1
        continue
...
# now add the morph counts to the table
wordix = 0
falsebadparses = [1]
badparseix = 0
inBAD = False
