...
# #enables experimentation on the connectivity files output by the meg_coherence_runner
...
with open('d1con.pkl','rb') as f:
  d1con = pickle.load(f)
with open('d2con.pkl','rb') as f:
  d2con = pickle.load(f)
with open('d3con.pkl','rb') as f:
  d3con = pickle.load(f)

d21con = d2con - d1con
n_rows, n_cols = d21con.shape[:2]
vmin = min(d21con.ravel())
vmax = max(d21con.ravel())
...
for i in range(n_rows):
  for j in range(n_cols):
    ...
    cax = axes[i, j].imshow(d21con[i, j, :], vmin=vmin, vmax = vmax, aspect = 'auto')
    if epochStart < 0:
    ...
    if i == (n_rows - 1):
      ...
      axes[i, j].set_xticks(numpy.arange(0.0,d21con.shape[3],62.5))
      axes[i, j].set_xticklabels(numpy.arange(epochStart,epochEnd,(epochEnd-epochStart)/(d21con.shape[3]/62.5)))
    axes[i, j].set_ylim([0.0, d21con.shape[2]-1])
fig.colorbar(cax)
                
if DEV:
  plt.savefig('graphics/coh_d21_dev.png')
else:
  plt.savefig('graphics/coh_d21_test.png')
  
d31con = d3con - d1con
n_rows, n_cols = d31con.shape[:2]
vmin = min(d31con.ravel())
vmax = max(d31con.ravel())
#vmin = -0.05
#vmax = 0.07
...
for i in range(n_rows):
  for j in range(n_cols):
    ...
    cax = axes[i, j].imshow(d31con[i, j, :], vmin=vmin, vmax = vmax, aspect = 'auto')
    if epochStart < 0:
      axes[i, j].axvline(x=epochStart *-1 * samplingRate, c='black', lw=1)
      #else: onset isn't on graph
    ...
    if i == (n_rows - 1):
      axes[i, j].set_xlabel(names[j])
      axes[i, j].set_xticks(numpy.arange(0.0,d31con.shape[3],62.5))
      axes[i, j].set_xticklabels(numpy.arange(epochStart,epochEnd,(epochEnd-epochStart)/(d31con.shape[3]/62.5)))
    axes[i, j].set_ylim([0.0, d31con.shape[2]-1])
fig.colorbar(cax)
if DEV:
  plt.savefig('graphics/coh_d31_dev.png')
else:
  plt.savefig('graphics/coh_d31_test.png')

d32con = d3con - d2con
n_rows, n_cols = d32con.shape[:2]
vmin = min(d32con.ravel())
vmax = max(d32con.ravel())
...
for i in range(n_rows):
  for j in range(n_cols):
    ...
    cax = axes[i, j].imshow(d32con[i, j, :], vmin=vmin, vmax = vmax, aspect = 'auto')
    if epochStart < 0:
      axes[i, j].axvline(x=epochStart *-1 * samplingRate, c='black', lw=1)
      #else: onset isn't on graph
    ...
    if i == (n_rows - 1):
      axes[i, j].set_xlabel(names[j])
      axes[i, j].set_xticks(numpy.arange(0.0,d32con.shape[3],62.5))
      axes[i, j].set_xticklabels(numpy.arange(epochStart,epochEnd,(epochEnd-epochStart)/(d32con.shape[3]/62.5)))
    axes[i, j].set_ylim([0.0, d32con.shape[2]-1])
fig.colorbar(cax)
if DEV:
  plt.savefig('graphics/coh_d32_dev.png')
else:
  plt.savefig('graphics/coh_d32_test.png')
