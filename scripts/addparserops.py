...
newcol = []
SKIP = 0
# first, connect all hyphenated words (keeping POS of last piece)
for i,l in enumerate(newcolFile):
    ...
    word = l.strip().split()
    if '-' == word[1]:
        newcol[-1][1] = newcol[-1][1] + word[1] + newcolFile[i+1].strip().split()[1] #keep POS of first piece
        newcol[-1][0] = word[0] #keep final POS
        SKIP = 1
        continue
...
# now add the parse ops to the table
wordix = 0
falsebadparses = [1]
badparseix = 0
inBAD = False
...
for tline in tableFile:
    ...
    if inBAD and '<SENT-END>' == stline[3]:
        ...
        # but then the parser ops would be off anyway
        inBAD = False
    if stline[0][0] == '#':
        #in a comment line
        stline.append('parseop')
        ...
        #in a badparse, so we don't know the op
        stline.append('-1')
        sys.stdout.write('\t'.join(stline)+'\n')
        continue
    elif stline[4] == "'":
        #take care of the occasional lone possessive apostrophe
        stline.append(newcol[wordix-1][0].split('-')[-1])
        sys.stdout.write('\t'.join(stline)+'\n')
        continue
    ...
    while americanize(badchars.sub('',newcol[wordix][1].lower())) != badchars.sub('',stline[4].lower()):
        ...
        if DEBUG:
            sys.stderr.write(str(wordix)+'/'+str(len(newcol))+': '+str(newcol[wordix])+' ?= ')
            sys.stderr.write(str(stline[4])+'\n')
    stline.append(newcol[wordix][0].split('-')[-1])
    sys.stdout.write('\t'.join(stline)+'\n')
    wordix += 1
